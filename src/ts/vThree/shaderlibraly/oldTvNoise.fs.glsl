...
#define PI 3.1415926535897932384626433832795
 ...
 void main(){
    vec2 uv = vec2(vUv.x, vUv.y);
    	...
    	float fuzzOffset = snoise(vec2(uTime*15.0,uv.y*80.0))*0.003;
    	float largeFuzzOffset = snoise(vec2(uTime*1.0,uv.y*25.0))*0.004;
        ...
        float yOffset = abs(sin(uTime)*4.0)*vertMovementOn+vertJerk*vertJerk2*0.1;
    	...
    	float red 	=   texture2D(	uTex, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
    	float green = 	texture2D(	uTex, 	vec2(uv.x + xOffset,	  y)).g+staticVal;
    	float blue 	=	texture2D(	uTex, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
